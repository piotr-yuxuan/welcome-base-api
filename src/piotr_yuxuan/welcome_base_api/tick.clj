...
(defn ^BigDecimal tick-radius+bod-price->tick-size
...
(comment
  (binding [*unchecked-math* :warn-on-boxed]
    (let [bod-price 100M]
      (c/quick-bench
        (bod-price+market-value->tick-value
          (tick-radius+bod-price->tick-size (tick-radius :percent) bod-price)
          bod-price
          101.5M)))

    (let [precision (inc (tick-order-of-magnitude :percent))
          ^int tick-radius (tick-radius :percent)
          bod-price 100M
          tick-size (tick-radius+bod-price->tick-size tick-radius bod-price)]
      (c/quick-bench
        (bod-price+market-value->tick-value
          tick-size
          bod-price
