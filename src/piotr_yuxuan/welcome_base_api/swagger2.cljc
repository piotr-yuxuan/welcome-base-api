(ns piotr-yuxuan.welcome-base-api.swagger2
...
(defn create-swagger-handler
  ...
  []
  (fn create-swagger
    ([{::r/keys [router match] :keys [request-method] :as thing0}]
     (let [{:keys [id] :or {id ::default} :as swagger} (-> match :result request-method :data :swagger)
           ...
           transform-endpoint (fn [[method {{:keys [coercion no-doc swagger] :as data} :data
                                            ...
                                            :as big-data}]]
                                (println :data (boolean data)
                                         :no-doc (boolean no-doc))
                                (if (and data (not no-doc))
                                  (let [temp (meta-merge
                                    ...
                                    (swap! debug conj {:big-data big-data
                                                       :ret ret})
                                    ret)))
